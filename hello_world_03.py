...
def process_log_file(cur, filepath):
    ...
    for i, row in time_df.iterrows():
        ...
        cur.execute(time_select, [row.ts.timestamp()])
        results = cur.fetchone()

        if results[0] == 0:
            list_row = list(row)
            list_row[0] = row.ts.timestamp()
            cur.execute(time_table_insert, list_row)
    ...
    for index, row in df.iterrows():
        ...
        cur.execute(
            song_select_by_song_id_artist_id, (row.song, row.artist, row.length)
        )
        results = cur.fetchone()
...
def main():
    connect = sqlite3.connect("sparkify.db")
    cursor = connect.cursor()

    process_data(cursor, connect, filepath="song_data/A", func=process_song_file)
    process_data(cursor, connect, filepath="log_data/2018/11", func=process_log_file)
